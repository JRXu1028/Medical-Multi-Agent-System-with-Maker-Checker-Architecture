...
def resolve_skills_dir(project_root: Path, skills_dir: Path = None) -> Path:
    """Return the configured Anthropic-style skills directory."""
    if skills_dir is None:
        return project_root / DEFAULT_SKILLS_DIR
    skills_dir = Path(skills_dir)
    return skills_dir if skills_dir.is_absolute() else project_root / skills_dir
...
def load_skill_function(
    ...
    resolved_skills_dir = resolve_skills_dir(project_root, skills_dir)
    module_path = resolved_skills_dir / skill_name / "script" / f"{script_name}.py"
...
def discover_skills(project_root: Path = None, skills_dir: Path = None) -> List[Dict]:
    """
    ...
    skills_dir = resolve_skills_dir(project_root, skills_dir)
    ...
    for skill_dir in skills_dir.iterdir():
        ...
        try:
            ...
            logger.debug(f"✅ Discovered skill: {skill_name} (function={function_name})")
        except (FileNotFoundError, AttributeError) as e:
            logger.warning(f"⚠️ Skipping {skill_name}: {e}")
            continue

    logger.debug(f"Discovered {len(discovered_skills)} skills in total")
    return discovered_skills


def load_all_skills(project_root: Path = None, skills_dir: Path = None) -> dict:
    """
    自动扫描并加载所有 Skills
    ...
    discovered = discover_skills(project_root, skills_dir)
