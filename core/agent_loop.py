...
class AgentLoop:
    ...
    async def run(self, agent, input_data: Dict[str, Any], session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 收集所有 Skill 调用结果，传递给 post_process_result
            # 让 Agent 在后期处理中基于结构化 Skill 输出生成 action_signal
            # 而非从自然语言中做 NLP 解析
            skill_results: list = []
                ...
                try:
                    ...
                    if llm_response.has_tool_calls():
                        ...
                        for tool_call in llm_response.tool_calls:
                            ...
                            # 收集结构化 Skill 结果，供 post_process_result 使用
                            skill_results.append({
                                "name": tool_call.name,          # Skill 函数名 (如 "assess_risk")
                                "arguments": tool_call.arguments, # 调用参数 (如 {"symptoms": "胸痛, 呼吸困难"})
                                "result": tool_result,           # 完整返回值 (含结构化字段如 risk_level)
                            })
                    ...
                    # 情况2: LLM 返回文本响应，任务完成
                        ...
                        # 让 Agent 进行结果后处理（基于结构化 Skill 返回值生成 action_signal）
                        # 传入 skill_results 使 Agent 可直接读取 Skill 的结构化字段
                        # 而非从自然语言中做 NLP 关键词解析
                        if hasattr(agent, 'post_process_result'):
                            result = await agent.post_process_result(
                                result, final_answer, skill_results=skill_results
                            )
