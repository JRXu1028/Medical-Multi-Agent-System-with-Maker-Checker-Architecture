...
2. 再审查 `process_trace.loaded_skills` 和 `process_trace.tool_trace` 是否支持本轮任务。
3. 再审查 `evidence_records` 是否支撑 `action_signal` 和最终回答。
4. 最后审查医疗安全、缺失上下文和输出边界。

## Issue Types

- TOOL_GAP: 工具路径、工具参数或 skill/tool 对应关系有问题。
- EVIDENCE_GAP: 证据不足、不支撑结论、过时或低相关。
...
- 不重复 deterministic precheck 已能稳定判断的硬规则。
